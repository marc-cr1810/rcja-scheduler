...
impl AppState {
    pub(super) fn draw_fields(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
                ...
                && !self.new_field_name.trim().is_empty()
            {
                let existing_ids: Vec<String> =
                    self.config.fields.iter().map(|f| f.id.clone()).collect();
                let id = crate::scheduler::unique_id(
                    &sanitize_name(&self.new_field_name),
                    &existing_ids,
                );
        ...
        egui::Grid::new("comp_fields_grid")
            .num_columns(3)
            ...
            .show(ui, |ui| {
                ...
                for (idx, field) in self.config.fields.iter_mut().enumerate() {
                    ...
                    if ui
                        .add_sized([220.0, 20.0], egui::TextEdit::singleline(&mut field.name))
                        .changed()
                    {
                        fields_changed = true;
                    }
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if let Some(ref mut allowed) = field.allowed_divisions {
                            ...
                            ui.label(
                                RichText::new("Allows All Divisions").color(theme::text_muted()),
                            );
                        }
                    });
        ...
        ui.horizontal(|ui| {
                ...
                && !self.new_table_name.trim().is_empty()
            {
                let existing_ids: Vec<String> =
                    self.config.fields.iter().map(|f| f.id.clone()).collect();
                let id = crate::scheduler::unique_id(
                    &sanitize_name(&self.new_table_name),
                    &existing_ids,
                );
                self.config.fields.push(Field {
                    id,
        ...
        egui::Grid::new("interview_tables_grid")
            .num_columns(2)
            .spacing(egui::vec2(20.0, 10.0))
            .striped(true)
            .show(ui, |ui| {
