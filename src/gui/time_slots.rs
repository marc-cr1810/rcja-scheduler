...
impl AppState {
    pub(super) fn draw_time_slots(&mut self, ui: &mut egui::Ui) {
        ...
        if self.config.day_configs.is_empty() {
            self.config
                .day_configs
                .push(crate::model::DayGenConfig::default());
        }
        ...
        egui::Grid::new("slots_grid")
            ...
            .show(ui, |ui| {
                ...
                for (idx, slot) in self.config.time_slots.iter().enumerate() {
                    ...
                    ui.label(
                        RichText::new(format!("{} {:?}", kind_icon, slot.kind)).color(kind_color),
                    );
    ...
    pub fn auto_generate_time_slots(&mut self) {
        ...
        for div in &self.config.divisions {
            ...
            let games = match div.mode {
                SchedulingMode::HeadToHead => {
                    let rr_matches = crate::scheduler::activity::compute_rr_match_count(
                        div_teams_count,
                        div.games_per_team,
                    );
                    let mut finals_matches = if div.finals_enabled {
                ...
                SchedulingMode::IndividualRun => div_teams_count * div.games_per_team,
            };
            total_games += games;
        ...
        if total_fields == 0 {
            self.status_message =
                "Error: Please add at least one field first to allocate slots!".to_string();
            return;
        }
        ...
        if required_slots == 0 {
            self.status_message =
                "Error: No teams or divisions defined to calculate matches!".to_string();
            return;
        }

        if self.config.day_configs.is_empty() {
            self.status_message =
                "Error: No day configurations specified for auto-generation!".to_string();
            return;
        }

        let mut old_day_slots = Vec::new();
        type VolunteerRanges = Vec<(String, Vec<(u32, u32)>)>;
        let mut volunteer_available_ranges: Vec<VolunteerRanges> =
            vec![Vec::new(); self.config.volunteers.len()];

        for day_config in &self.config.day_configs {
            let target_day_lower = day_config.day.to_lowercase();
            let day_slots: Vec<(String, u32, u32)> = self
                .config
                .time_slots
                .iter()
                ...
                .map(|s| {
                    (
                        s.id.clone(),
                        s.start_minutes(),
                        s.start_minutes() + s.duration_minutes(),
                    )
                })
                .collect();

            for (vol_idx, vol) in self.config.volunteers.iter().enumerate() {
                let ranges: Vec<(u32, u32)> = day_slots
                    .iter()
        ...
        for day_config in &self.config.day_configs {
            ...
            self.config
                .time_slots
                .retain(|s| s.day.to_lowercase() != target_day_lower);
        }
        ...
        for day_config in &self.config.day_configs {
            ...
            if start_parts.len() != 2 {
                self.status_message = format!(
                    "Error: Invalid start time format for {}. Use HH:MM.",
                    day_config.day
                );
                return;
            }
            ...
            let mut h = match start_parts[0].parse::<u32>() {
                ...
                Err(_) => {
                    self.status_message =
                        format!("Error parsing start hour for {}.", day_config.day);
                    return;
                }
            };
            let mut m = match start_parts[1].parse::<u32>() {
                ...
                Err(_) => {
                    self.status_message =
                        format!("Error parsing start minute for {}.", day_config.day);
                    return;
                }
            };
            ...
            if end_parts.len() != 2 {
                self.status_message = format!(
                    "Error: Invalid end time format for {}. Use HH:MM.",
                    day_config.day
                );
                return;
            }
            ...
            let end_h = match end_parts[0].parse::<u32>() {
                Ok(val) => val,
                Err(_) => {
                    self.status_message = format!("Error parsing end hour for {}.", day_config.day);
                    return;
                }
            };
            let end_m = match end_parts[1].parse::<u32>() {
                ...
                Err(_) => {
                    self.status_message =
                        format!("Error parsing end minute for {}.", day_config.day);
                    return;
                }
            };
            ...
            if day_config.lunch_enabled {
                ...
                if lunch_parts.len() == 2
                    && let (Ok(lh), Ok(lm)) =
                        (lunch_parts[0].parse::<u32>(), lunch_parts[1].parse::<u32>())
                {
            ...
            loop {
                ...
                if day_config.lunch_enabled
                    && current_start_minutes < lunch_end_minutes
                    && current_end_minutes > lunch_start_minutes
                {
                ...
                let next_start_minutes =
                    current_start_minutes + slot_duration + self.gen_match_slot_break;
            ...
            if day_config.interviews_enabled {
                ...
                loop {
                    ...
                    let next_start_minutes =
                        current_start_minutes + int_slot_duration + self.gen_interview_slot_break;
        ...
        for (vol_idx, vol) in self.config.volunteers.iter_mut().enumerate() {
            let old_slot_ids: std::collections::HashSet<&str> =
                old_day_slots.iter().map(|(id, _, _)| id.as_str()).collect();
            vol.availabilities
                .retain(|id| !old_slot_ids.contains(id.as_str()));
            ...
            for (day_name, old_ranges) in &volunteer_available_ranges[vol_idx] {
                ...
                let day_new_slots: Vec<(String, u32, u32)> = self
                    .config
                    .time_slots
                    .iter()
                ...
                for (new_id, new_start, new_end) in &day_new_slots {
                    let overlaps = old_ranges
                        .iter()
                        .any(|(old_start, old_end)| new_start < old_end && new_end > old_start);
